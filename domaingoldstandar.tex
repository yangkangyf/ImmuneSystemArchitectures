...
and BluePippin size selection cutoff of 7kb or not size selection in order to
prepare  3 libraries of 20kb SMRTbell to be run on 11 SMRT cells to produce
...
Illumina sequencing followed the procedure discribed in (Velandia et al) except
that due to a potentially higher error rate in the PacBio data, all read with a
PhRed value of $ge$10 were retained and procesed via kmer matching by BBDuk.
Finally 10.5 Gpb of paired reads and 1.3Gbp single reads were used to correct
PacBio reads in further steps.
\subsubsection*{Sequence data processing}
In order to improve and complete the genome assemby an hybrid and non
...
without HPGA correction was also filtered by using \textbf{dextract} (reads of a
...
sequencing was performed by proovread-2.13.13.
\subsubsection*{Genome assembly}
Celera Assembler 8.3rc2 was used to assembly the three set of reads with error
...
Poner los nombres y parametros del ensamblaje del Transcriptoma con trinity
