...
domains ($w_1 \rightarrow w_2$). Both, N and C-terminus are central nodes but 
with opposite order relations, as expected due the arrangement inside the bi-gram. 

%Another sets of frequent relations are composed by: PF00090
%(Thrombospondin type 1 domain, always as $w_1$), PF01826 
%(Trypsin Inhibitor like cysteine rich domain, both), PF00094 
%(Willebrand factor type D domain, both) and PF08742 (C8 domain, both).

%PF13855 (Leucine rich repeat, both) as a central node, 
%PF01463 (Leucine rich repeat C-terminal domain, both),
%PF01462 (Leucine rich repeat N-terminal domain, both),
%N-terminal (as $w_1$) and C-terminal (as $w_2$).

%PF00041 (Fibronectin type III domain, both),
%PF07679 (Immunoglobulin I-set domain, both),
%PF13927 (Immunoglobulin domain Ig 3, both),
%PF13895 (Immunoglobulin domain Ig 2, as $w_2$).

%PF00169 (Pleckstrin homology domain, both),
%PF00621 (RhoGEF domain,both).
...
\begin{figure}[ht!]
  \centering
  \includegraphics[scale=0.3]{figures/network_10_bigrams}
