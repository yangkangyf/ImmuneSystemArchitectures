...
We suspect, therefore, that the chordate immune sysm.31051tem also has undergone
...
\item[\textbf{O}rder] \TODO{What exactly is the match criterion?: Do you need
    ...
    \CAVH{Refers to the comparison of the $Q_n$ and $Pn(a)$ domains. It is 
required that order is preserved and also the domain lists have to be the 
same. Repetitions from query and subject are represented by one domain.}
\item[\textbf{Disorder}] \TODO{For the writing below you require that the target
  ...
  \CAVH{The second case refers to set comparisons between $Q_n$ and $P_m(a)$. 
The number of common elements ($Q_n \cap P_m(a)$) must be the same number of 
elements in $Q_n$ and in $P_m(a)$. In this case, the order does not be 
preserved.} 
...
from \textbf{O}.} \CAVH{Because Order and Disorder strategies consider 
proteins with $> 2$ domains, it is required to detect those candidates with 
only one domain family. Here was used the implementation by the RADS program. 
That allow the comparison of architectures and provide the comparison of 
domain strings based on a Pfam v.30 identity matrix score \cite{Terrapon:2014}. 
It is useful to detect not only homodomain proteins, but also homology 
relations between domains with different accession numbers.}  
\end{itemize}

The different annotation systems and comparison methods have been
integrated into a single workflow summarized in
Fig.~\ref{fig:workflow_golden}.
...
%\left|Q^a_{m}\right| \geq 2 \\ Re-defined it was wrong written.
    1, & \left|Q^a_{m} \cap \boldsymbol{\mathfrak{G}}\right| = 
\left|Q^a_{m}\right| = \left|\boldsymbol{\mathfrak{G}}\right| \quad and \quad 
\left|Q^a_{m}\right| \geq 2 \\
    0, & \textsl{otherwise}
  \end{array}\right.
\end{equation}
...
% Please evaluaate if this math description is correct! I just wanted to point 
