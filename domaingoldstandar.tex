...
estimated values changed: (median =$38.565$\% $\pm 12.23$, n=$8$). Specifically, inside this 
clade, through colonial (median=$39.66$\% $\pm 15.61$, n=$3$) and solitary 
($37.47$\% $\pm 9.20$, n=$5$) tunicates there is not significant differences on the 
...
The biological meaning of the domain homology strategies is explained on 
Table~\ref{table:biologicalexplanation}. Relationships between subject
proteins and their detected $\boldsymbol{\mathfrak{G}}$ proteins have explained
by one or many ABDO comparisons for the same pair of proteins, it means that
for each protein could exists one or more $\boldsymbol{\mathfrak{G}}$ proteins that
have been detected by exclusively one ABDO method or more. In the first case, is
possible to analyze the influence of defined methodologies to find new innate
immune system candidates. As shown in Figure~\ref{fig:relation}, a relationship
between the number of $\boldsymbol{\mathfrak{G}}$ proteins and $\boldsymbol{\mathfrak{G}}$
species, was calculated and then the complete number of subject species was 
used to compare all of the ABDO strategies. As explained earlier, those strategies
could be grouped by the number of coincident strategies that detected the same relation 
(gray region on the top of plot). 
...
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.5]{figures/relationGoldenABDO}
